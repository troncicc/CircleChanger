...
class CircleChanger(object):
    ...
    def swell_or_shrink_repeatedly(self,
        ...
        for k in range(2 * times_to_swell_or_shrink):
            ...
            amount_to_swell_or_shrink = amount_to_swell_or_shrink * -1
            self.draw()
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        mid = rg.Point.halfway_to(other_circle_changer.circle.center, self.circle.center)
        rad = self.get_distance_from(other_circle_changer.circle.center)
        new_circle = CircleChanger(self.circle.center.x, self.circle.center.y, rad, 'red', other_circle_changer.colors)
        new_circle.circle = rg.Circle(mid, rad)
